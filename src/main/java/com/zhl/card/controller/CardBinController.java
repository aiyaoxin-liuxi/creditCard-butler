...
public class CardBinController extends BaseController{
	...
	public Object queryCardBin(String cardNo){
        ...
        if(null != cardBin){
            ...
            // 从缓存银行卡表中获取银行名称，若有则替换传入名称
            Object bankObj = MemcacheManager.getInstance().get(Constant.BANK_MAP);
            if(Stringer.isNullOrEmpty(bankObj)){
                cardBinService.cardBinInit();
                object = MemcacheManager.getInstance().get(Constant.BANK_MAP);
            }
            if(Stringer.isNullOrEmpty(bankObj)){
                resEntity.setResponseEntity(keySn, ReturnCode.CACHE_FAIL_200003.getCode(), ReturnCode.CACHE_FAIL_200003.getName());
                return resEntity;
            ...
            String bankName = (String) bankMap.get(cardBin.getBankinfoId());
            if(!Stringer.isNullOrEmpty(bankName)){
                cardBin.setIssincName(bankName);
            }
