...
public enum EmailCode {
	...
	/** 短信通道返回成功 */
	CODE111111("111111","返回成功"),
	
	/** 数据验证失败 */
	CODE200000("200000","数据验证失败"),
	/** 验证码验证失败 */
	CODE200001("200001","验证码验证失败"),
	/** 上传参数验证失败： */
	...
	public static void main(String[] args) {
	 System.out.println(EmailCode.parseOf("00").getName());	
	}
	
	private EmailCode(String code,String name){
		this.code=code;
		this.name=name;
	}
	...
	private static Map<String, EmailCode> valueMap = new HashMap<String, EmailCode>();

	static {
		for (EmailCode _enum : EmailCode.values()) {
			valueMap.put(_enum.code, _enum);
		}
	}
	
	public static EmailCode parseOf(String code) {
		for (EmailCode item : values())
